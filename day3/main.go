...
func walkPath(direction string, distance int, start *point) ([]point, point) {
	cursor := point{x: start.x, y: start.y}
	coords := make([]point, distance+1)
	switch direction {
	case "R":
	...
	coords[distance] = cursor
	return coords, cursor
}

func getPathsForCoords(coords []string) [][]point {
	cursor := point{x: 0, y: 0}
	path := []point{}
	paths := make([][]point, len(coords))
	for index, coord := range coords {
		direction, distance := convertCoord(coord)
		path, cursor = walkPath(direction, distance, &cursor)
		paths[index] = path
	}
	return paths
}

func convertPathsToMap(paths [][]point, score int) map[point]int {
	pathMap := map[point]int{}
	for _, path := range paths {
		for _, cursor := range path {
			pathMap[cursor] |= score
		}
	}
	return pathMap
}

func findSmallestDistanceForIntersections(intersections map[point]bool) int {
	distance := -1
	for coord := range intersections {
		x := int(math.Abs(float64(coord.x)))
		y := int(math.Abs(float64(coord.y)))
		if x+y < distance || distance == -1 {
			distance = x + y
			fmt.Println("Found distance:", distance)
		}
	}
	return distance
}

func findUniqueIntersections(pathMaps []map[point]int) map[point]bool {
	intersections := make(map[point]bool)
	for i := 0; i < len(pathMaps); i++ {
		for j := 0; j < len(pathMaps); j++ {
			if i != j {
				for key := range pathMaps[j] {
					_, ok := pathMaps[i][key]
					if ok {
						_, intersectionExists := intersections[key]
						if !intersectionExists && key.x != 0 && key.y != 0 {
							fmt.Println("Found intersection at:", key)
...
// CalculateNearestIntersectionDistance finds the closest Manhattan distance
// value for a set of paths that intersect
func CalculateNearestIntersectionDistance(allPaths [][]string) int {
	pathMaps := make([]map[point]int, len(allPaths))

	for index, path := range allPaths {
		paths := getPathsForCoords(path)
		pathMap := convertPathsToMap(paths, 1)
		pathMaps[index] = pathMap
	}

	intersections := findUniqueIntersections(pathMaps)
	nearestIntersection := findSmallestDistanceForIntersections(intersections)

...
func TestCalculateNearestIntersectionDistance(t *testing.T) {
	table := []struct {
		input    [][]string
		expected int
	}{
	...
	for _, row := range table {
		result := CalculateNearestIntersectionDistance(row.input)
		if result != row.expected {
			t.Errorf("Failure comparing intersection distance! Expected %d, received %d!", row.expected, result)
		}
	}
}
